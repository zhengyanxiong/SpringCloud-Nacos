package io.agilefast.gateway.security;

import io.agilefast.gateway.exception.ErrorCode;
import io.agilefast.gateway.exception.ServerException;
...
public class CustomRemoteTokenServices {
    ...
    private Map<String, Object> postForMap(String path, MultiValueMap<String, String> formData, HttpHeaders headers) {
        ...
        try {
            ...
            log.error("catch token exception when check token!", e1);
            map.put(ERROR, e1.getStatusCode());

        } catch (HttpServerErrorException e2) {
            log.error("catch no permission exception when check token!", e2);
            map.put(ERROR, e2.getStatusCode());

        } catch (Exception e) {
            log.error("catch common exception when check token!", e);
        }
